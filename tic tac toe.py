...
class Grafic_Board(App):
    ...
    def build(self):
        ...
        for i in range(3):
            for j in range(3):
                button = Button(background_normal='tic tac toe background.jpg')
    ...
    def human_play(self, instance):
        async def human_play():
            ...
            self.buttons[self.row][self.cul].background_normal = 'tic tac toe round.jpg'
    ...
    def computer_play(self, str):
        async def computer_play(str):
            ...
            self.board[i][j] = 2
            bool = False
            self.buttons[i][j].background_normal = 'tic tac toe x.jpg'
            str = np.array2string(self.board)
            str = self.change_str(str)
            self.lst.append(str)
